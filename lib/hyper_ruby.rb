...
#                                      request (default 4)
#
# Syslog messages go to the same Server#run_worker block as requests, which tests
# what it was given: a HyperRuby::Request is answered with a Response, while a
# HyperRuby::SyslogMessage (readers message, peer_ip, transport, received_at_ns,
# message_id, attempt) is answered with an admission verdict. transport is :stream
# or :datagram; peer_ip is nil when the transport names no peer, such as a Unix
# socket connection with no PROXY header. message_id is stable across the retries
# of one message and unique for the life of the process, and attempt starts at 1,
# so a block can make its retries idempotent.
#
